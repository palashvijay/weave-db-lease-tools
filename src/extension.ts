...
async function listRepos(octokit: Octokit, org: string) {
...
async function fetchWeaveYaml(octokit: Octokit, org: string, repo: string) {
    ...
    let disposable = vscode.commands.registerCommand("weave.generateLeaseCommand", async () => {
        ...
        if (!token) {
            vscode.window.showErrorMessage(
                "Could not retrieve GitHub token. Please run `gh auth login` first."
            );
            return;
        }
        ...
        const configObj: any = yaml.load(yamlText);
        ...
        const getDefault = (key: string) =>
            defaults.find((d: any) => d.name === key)?.value;
